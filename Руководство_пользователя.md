...
Теперь такое безопасное поведение стало режимом по умолчанию, но его можно переключать пятью опциональными флагами в JSON:
...
- если `use_ai_optimal_then_identity_safe_framing = true`, из одного изображения строится одно видео, где первая половина идет как AI-optimal, а вторая половина плавно переводит камеру в identity-safe режим с более безопасной дистанцией;
...
Одновременно можно включать только один из этих пяти флагов.
...
- `use_ai_optimal_then_identity_safe_framing` — по умолчанию `false`; создавать одно видео, где первая половина идет в AI-optimal режиме, а вторая половина переходит в identity-safe дистанцию и ракурсы.
...
- JSON-выход имеет строгий формат из одного элемента: `[{"lang":"en","prompt":"..."}]`.
