...
def test_openai_synthesizer_prompt_uses_ai_optimal_framing_when_requested() -> None:
    ...
    assert "do not let the face become significantly enlarged or distorted" in prompt_text


def test_motion_selector_prompt_can_request_hybrid_ai_optimal_then_identity_safe_framing() -> None:
    metadata = _build_metadata()
    scene = SceneAnalysis(
        summary="Adult portrait with one clearly visible face.",
        people_count=1,
