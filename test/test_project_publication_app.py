...
def test_write_publication_bundle_sanitizes_paths_inside_source_files() -> None:
    ...
    (root / "config.json").write_text('{"key": "value"}\n', encoding="utf-8")
    _write_required_docs(root)
    (root / "api" / "paths.py").write_text('SOURCE = "E:\\\\Sensitive\\\\folder\\\\file.txt"\n', encoding="utf-8")
    (root / "utils" / "paths.py").write_text("TARGET = E:\\\\Work\\\\bundle\\\\out\n", encoding="utf-8")
    (root / "project_structure_registry.json").write_text(
        json.dumps(
            {
                "project": "sample",
    ...
    assert "E:\\Sensitive" not in api_text
    assert "E:\\Work" not in utils_text
