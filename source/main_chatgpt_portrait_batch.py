...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        dest="result_timeout",
        help="Seconds to wait for each generated portrait/image. Defaults to portrait config result_timeout or 300.",
    )
...
def load_portrait_config(path: Path) -> PortraitBatchConfig:
    ...
    result_timeout = _optional_positive_float(data.get("result_timeout"), "result_timeout")
...
def _optional_positive_float(value: Any, field_name: str) -> Optional[float]:
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise PortraitConfigError(f"Portrait config '{field_name}' must be a positive number.")
    parsed = float(value)
    if parsed <= 0:
        raise PortraitConfigError(f"Portrait config '{field_name}' must be greater than zero.")
    return parsed


DEFAULT_RESULT_TIMEOUT_SEC = 300.0


def _resolve_result_timeout(args: argparse.Namespace, portrait_config: PortraitBatchConfig) -> float:
    cli_timeout = getattr(args, "result_timeout", None)
    if cli_timeout is not None:
        if cli_timeout <= 0:
            raise PortraitConfigError("--result-timeout/--timeout must be greater than zero.")
        return float(cli_timeout)
    if portrait_config.result_timeout is not None:
        return portrait_config.result_timeout
    return DEFAULT_RESULT_TIMEOUT_SEC
...
def run_batch(
    ...
    args.result_timeout = _resolve_result_timeout(args, portrait_config)
