...
def test_run_generation_creates_identity_safe_and_closeup_prompt_variants() -> None:
    root = Path("test_runtime") / f"scene_pipeline_closeup_dual_{uuid4().hex}"
    input_dir = root / "input"
    output_dir = root / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    image_path = input_dir / "frame.png"
    Image.new("RGB", (160, 240), (120, 100, 90)).save(image_path)
    ...
    args = Namespace(
        image=image_path,
        stage_id="scene_stage_closeup_dual",
        scene_model="gpt-4.1",
        generate_video=False,
        generate_styled_images=False,
        prompt_model="gpt-4.1",
    ...
    _run_generation(
        ...
        GenerationConfig(
            ...
            motion_source=MotionSource.TABLE,
            generate_final_frames=False,
            generate_identity_safe_closeup_videos=True,
        ),
        settings=settings,
        scene_analyzer=fake_scene_analyzer,
        prompt_synthesizer=fake_prompt_synthesizer,
    ...
    assert seen_modes == [VideoFramingMode.IDENTITY_SAFE, VideoFramingMode.FACE_CLOSEUP]
    assert (output_dir / "scene_stage_closeup_dual_v_prompt_1.txt").read_text(encoding="utf-8") == "mode=identity_safe"
    assert (output_dir / "scene_stage_closeup_dual_v_prompt_2.txt").read_text(encoding="utf-8") == "mode=face_closeup"


def test_write_pipeline_manifest_tracks_identity_safe_closeup_steps() -> None:
    root = Path("test_runtime") / f"framing_manifest_closeup_dual_{uuid4().hex}"
    root.mkdir(parents=True, exist_ok=True)
    ...
    manifest = manifest_path.read_text(encoding="utf-8")
    assert '"generate_identity_safe_closeup_videos": true' in manifest
    assert '"framing_mode": "identity_safe"' in manifest
    assert '"framing_mode": "face_closeup"' in manifest


def test_run_generation_creates_single_prompt_variant_in_hybrid_framing_mode() -> None:
    root = Path("test_runtime") / f"scene_pipeline_hybrid_{uuid4().hex}"
    input_dir = root / "input"
    output_dir = root / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    image_path = input_dir / "frame.png"
    Image.new("RGB", (160, 240), (125, 105, 88)).save(image_path)
    ...
    def fake_scene_analyzer(_image_path: Path, _model: str | None) -> SceneAnalysis:
        return SceneAnalysis(
            ...
            background="street",
            shot_type="medium shot",
            main_action="the subject turns gently",
            mood=["calm"],
            relationships=[],
        )
    ...
    assert seen_modes == [VideoFramingMode.AI_OPTIMAL_THEN_IDENTITY_SAFE]
    assert (output_dir / "scene_stage_hybrid_v_prompt_1.txt").read_text(encoding="utf-8") == "mode=ai_optimal_then_identity_safe"


def test_write_pipeline_manifest_tracks_hybrid_framing_mode() -> None:
    root = Path("test_runtime") / f"framing_manifest_hybrid_{uuid4().hex}"
    root.mkdir(parents=True, exist_ok=True)
...
def test_run_generation_creates_three_prompt_variants_in_triple_framing_mode() -> None:
    root = Path("test_runtime") / f"scene_pipeline_triple_{uuid4().hex}"
    input_dir = root / "input"
    output_dir = root / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    image_path = input_dir / "frame.png"
    Image.new("RGB", (160, 240), (130, 110, 95)).save(image_path)
    ...
    assert (output_dir / "scene_stage_triple_v_prompt_1.txt").read_text(encoding="utf-8") == "mode=identity_safe"
    assert (output_dir / "scene_stage_triple_v_prompt_2.txt").read_text(encoding="utf-8") == "mode=face_closeup"
    assert (output_dir / "scene_stage_triple_v_prompt_3.txt").read_text(encoding="utf-8") == "mode=ai_optimal"


def test_write_pipeline_manifest_tracks_triple_framing_steps() -> None:
    root = Path("test_runtime") / f"framing_manifest_triple_{uuid4().hex}"
    root.mkdir(parents=True, exist_ok=True)
    ...
    manifest = manifest_path.read_text(encoding="utf-8")
    assert '"generate_triple_framing_videos": true' in manifest
    assert '"framing_mode": "identity_safe"' in manifest
    assert '"framing_mode": "face_closeup"' in manifest
    assert '"framing_mode": "ai_optimal"' in manifest
