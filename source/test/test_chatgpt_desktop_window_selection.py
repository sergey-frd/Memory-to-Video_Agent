...
def test_wait_for_result_image_waits_until_generation_indicator_clears(monkeypatch) -> None:
    agent = ChatGPTDesktopAgent(
        DesktopAgentConfig(
            image_path=None,
            prompt_text="prompt",
            min_result_wait_sec=0.0,
            result_stable_sec=0.0,
            result_timeout_sec=30.0,
            verbose=True,
        )
    )
    window = _FakeWindow("ChatGPT - Google Chrome")
    generation_states = iter([True, True, False, False])
    clock = {"value": 98.0}
    accepted_signature = (10, 20, 300, 400, 12345)
    accepted_wrapper = object()

    def _advance_time(_seconds: float = 0.0) -> float:
        clock["value"] += 2.0
        return clock["value"]

    monkeypatch.setattr(agent, "_wait_until_generation_settles", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(agent, "_has_generation_running_indicator", lambda _window: next(generation_states))
    monkeypatch.setattr(agent, "_find_result_images", lambda _window: [accepted_wrapper])
    monkeypatch.setattr(agent, "_result_signature", lambda _wrapper: accepted_signature)
    monkeypatch.setattr(
        agent,
        "_signature_matches_baseline",
        lambda *_args, **_kwargs: False,
    )
    monkeypatch.setattr("api.chatgpt_desktop_v2.time.sleep", lambda _seconds: None)
    monkeypatch.setattr("api.chatgpt_desktop_v2.time.time", _advance_time)

    wrapper, signature = agent._wait_for_result_image(window, [])

    assert wrapper is accepted_wrapper
    assert signature == accepted_signature


def test_collect_submission_baseline_signatures_deduplicates_attachment_and_result(monkeypatch) -> None:
    agent = ChatGPTDesktopAgent(_config())
    window = _FakeWindow("ChatGPT - Google Chrome")
    shared_wrapper = object()
    attachment_only = object()
    signatures = iter([(1, 2, 3, 4, 11), (5, 6, 7, 8, 22), (1, 2, 3, 4, 11)])

    monkeypatch.setattr(agent, "_find_result_images", lambda _window: [shared_wrapper])
    monkeypatch.setattr(agent, "_find_attachment_image_candidates", lambda _window: [shared_wrapper, attachment_only])
    monkeypatch.setattr(agent, "_result_signature", lambda _wrapper: next(signatures))

    collected = agent._collect_submission_baseline_signatures(window)

    assert collected == [(1, 2, 3, 4, 11), (5, 6, 7, 8, 22)]
