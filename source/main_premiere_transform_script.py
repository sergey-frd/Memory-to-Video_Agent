...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a Premiere ExtendScript file that applies Transform effects and scale keyframes."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional optimizer JSON whose transform_plan entries should drive per-clip effects.",
    )
    parser.add_argument("--default-effect-name", type=str, default="Transform", help="Fallback Premiere effect name.")
    parser.add_argument("--track-index", type=int, default=0, help="Zero-based Premiere video track index.")
    parser.add_argument(
        "--add-video-effects",
        action="store_true",
        help="Also add named Premiere Transform-category effects. Default is safer Motion Scale keyframes only.",
    )
    parser.add_argument(
        "--no-safe-transform-effect",
        action="store_true",
        help="Do not add the plain Transform effect; keyframe only the intrinsic Motion Scale parameter.",
    )
...
def main() -> None:
    ...
    if args.config is not None:
        ...
        output_path, jobs = write_premiere_transform_extendscript(
            ...
            default_effect_name=args.default_effect_name,
            add_video_effects=args.add_video_effects,
            apply_safe_transform_effect=not args.no_safe_transform_effect,
        )
        output_paths = [(output_path, len(jobs))]

    for output_path, job_count in output_paths:
        print(f"Generated Premiere transform script: {output_path} ({job_count} transform job(s))")
...
def _write_from_batch_config(args: argparse.Namespace) -> list[tuple[Path, int]]:
    ...
    add_video_effects = args.add_video_effects or bool(payload.get("premiere_transform_script_add_video_effects", False))
    if args.no_safe_transform_effect:
        apply_safe_transform_effect = False
    else:
        apply_safe_transform_effect = bool(payload.get("premiere_transform_script_apply_safe_effect", False))
    default_effect_name = str(payload.get("premiere_transform_script_default_effect_name") or args.default_effect_name)
    track_index = int(payload.get("premiere_transform_script_track_index", args.track_index))
    save_project = bool(payload.get("premiere_transform_script_save_project", not args.no_save_project))
    for index, raw_job in enumerate(sequence_jobs, start=1):
        ...
        output_jsx_path = reports_dir / f"{_slugify_filename(sequence_name)}_apply_transforms.jsx"
