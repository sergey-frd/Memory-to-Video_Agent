...
## 2026.09.22.01 — первая сборка сразу компактная

- Принцип достаточного времени для каждого видео, фото и ART действует с первой структуры после CLASSIFY и в редакциях FULL/MINIMAL.
- По умолчанию компактный план допускает меньшую длительность без заполнения недобора. Убраны добавление секунд фото, процентная подгонка и удаление финальных сцен ради лимита.
- Обоснования элементов сохраняются в плане; точные смысловые In/Out остаются предметом просмотра материала.
- Согласованы документация, примеры конфигураций и версии. Публичный пакет исключает персональные аудиты, сборщики и отчёты Ben26/Arkady26.
- Обновление предназначено для будущих проектов. Существующие проекты героев не пересобирались; Adobe и генерация видео при выпуске не запускаются.

## 20.09.2026 — FULL + MINIMAL и завершение производства

- Описаны две редактируемые 4K-последовательности в одном проекте, фиксация FULL перед новым MINIMAL, контрольные копии ASSEMBLY/MOTION и сравнение сохранённого FULL после сборки короткой версии.
- Зафиксирован совместный порядок Adobe: агент готовит проверенный JSX и монитор, пользователь запускает; подтверждение берётся из нативных журналов и сохранённого проекта.
- Добавлены выраженные, чередующиеся профили движения ART, сохранение всех заказанных работ и проверка запасов материала для переходов.
- Closeout поддерживает точный проверяемый список назначения файлов и удаление ненужных промежуточных результатов без их архивирования. Исходники, классификации и зависимости сохраняемых проектов защищены.
- Обновлена граница публичной поставки: персональные сборщики и локальные отчёты не включаются в общий пакет.

Краткая история заметных пользовательских изменений. Подробные инструкции находятся в [`docs/`](docs/).

## 16.09.2026 — подготовлено локально, публикация не выполнялась

- Добавлен повторяемый closeout: план, архивирование, SHA-256-проверка, очистка и постоянный отчёт. Принятые финалы и используемые медиа защищены.
- Добавлено завершение после ручного переноса медиа: новые `_RELINKED` копии, проверка постоянного архива и удаление перечисленных рабочих остатков. Структурная проверка не заменяет Adobe open-check.
- `run_prepare_public_bundle.bat` собирает отдельный пакет в новую папку и проверяет манифест, приватные файлы, шаблоны секретов и синтаксис. Git-операции не выполняются.
- Из публикации исключены личные конфиги, отчёты и данные заданий; исправлена обработка обоих видов Windows-путей.
- В плане производства AI включён по умолчанию для анализа, классификации и структуры, с возможностью отключения и повторного использования кэша. Общая производственная цепочка остаётся проектируемой.
- Обновлены руководства и HTML; очищены временные тестовые данные и устаревшие сборки. Архивы и действующий публичный пакет сохраняются.

## 2026.08.31.01

- Интегрированы TASK_031–034, QA-помощники и native JSX TASK_032 отдельно от личных артефактов.
- Добавлены переносимые JSON-профили и `run_premiere_art_task.bat`: read-only check по умолчанию, явный execute, защита предыдущих результатов.
- `install_project.bat` устанавливает Python-зависимости из desktop lock; setup останавливается при ошибке и сохраняет локальные настройки.
- `run_verify_installation.bat --require-tag` сверяет выпуск, Git, Python, версии пакетов и доступность FFmpeg.
- Добавлены руководство установки на ноутбук и подробная инструкция ART; обновлены RU/EN guides, BAT-матрица и HTML.
- Публичный пакет включает JSX и пустой `.env.template`, исключает `*.local.json` и новые TASK-каталоги.
- Монтажные результаты не перегенерируются этим релизом; Desktop QA на новом компьютере остаётся отдельной проверкой.
- Известные ограничения: в полном наборе остаются 10 ошибок старых тестов ChatGPT desktop/portrait, sequence optimizer и video prompt story. Все 10 воспроизведены на предыдущем коммите `291557c`; это не новые регрессии ART/установки. Новые проверки и чистая установка проходят.
- Известные ограничения: в полном наборе остаются 10 ошибок старых тестов ChatGPT desktop/portrait, sequence optimizer и video prompt story. Все 10 воспроизведены на предыдущем коммите `291557c`; это не новые регрессии ART/установки. Новые проверки и чистая установка проходят.
