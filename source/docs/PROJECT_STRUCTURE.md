...
| Sequence Trim Review | `main_sequence_trim_review.py`, `utils/sequence_trim_*.py`, `utils/premiere_trim_review_export.py`, `utils/video_frame_extract.py`, `api/openai_trim_semantic.py`, `models/sequence_trim_review.py` | KEEP/DROP сегменты внутри клипов (heuristic + semantic), compact keep | review `.prproj`, reports |
