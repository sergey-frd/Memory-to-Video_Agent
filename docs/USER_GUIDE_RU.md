...
[`config_api_single_image.example.json`](../source/examples/config_api_single_image.example.json),
...
- `premiere_project_dir` — необязательный путь к папке проекта Adobe Premiere (не к файлу `.prproj`). Задаётся в личном `config_Hero.local.json`; сохраняйте историческое расположение проектов героя.
- `project_video_exports_dir` — необязательная папка MP4, экспортированных из монтажного проекта: промежуточные версии для просмотра и принятые финальные ролики. Историческое имя папки может не совпадать с годом текущего задания. Это отдельное назначение от `final_videos_dir`, где сохраняются AI-видеофрагменты. Параметр сохраняется при загрузке и override, по умолчанию `None`; автоматический экспорт или перенос не запускает. При ручном экспорте пользователь выбирает эту папку. Промежуточные MP4 остаются кандидатами на последующую очистку после принятия финала.
- `source_materials_dir` — необязательный путь к корневой папке всех исходных материалов текущего поздравления. Пути описывают контекст задания, сохраняются при загрузке и override; сами по себе не меняют вход pipeline, доставку результатов и не запускают сканирование или перенос файлов. Если не заданы, имеют значение `None`.
