...
| Добавить минимальный intrinsic Motion в копию sequence | `main_premiere_import_keep.py`, режим `premiere_sequence_motion_animation` | `run_premiere_sequence_motion.bat` | `premiere_sequence_motion_template.json` | Save As `.prproj`, новая sequence, dry-run JSON, QA TXT, silent review MP4 |
| Вставить диапазон из другой sequence и анимировать статичные кадры | `main_premiere_import_keep.py`, режим `premiere_sequence_insert_from_sequence_and_motion_animation` | `run_premiere_sequence_motion.bat` | `premiere_sequence_insert_motion_template.json` | Save As `.prproj`, новая milestone sequence, frame-exact video-only insert, Motion, dry-run/QA, silent review MP4 |
