...
```powershell
python .\main_project_publication.py --target-dir "C:\Path\To\Memory-to-Video_Agent"
```

Inspect the diff in the publication repo before pushing:

```powershell
cd "C:\Path\To\Memory-to-Video_Agent"
git status
git diff --stat
```
...
python .\main_project_publication_push.py `
  --repo-dir "C:\Path\To\Memory-to-Video_Agent" `
  --source-root . `
